...
(deftest list-latest-outcomes-for-session-returns-latest-per-dispatch-test
  (tc.pg/with-postgres [pg]
    (let [jdbc-url (tc.pg/jdbc-url pg)
          ...
          wh-2 (UUID/randomUUID)
          _ (jdbc/execute! ds ["INSERT INTO webhooks (id, tenant_id, name, url, enabled, auth_type, created_at)
                               VALUES (?, ?, ?, 'http://example.invalid/1', true, 'none', now()),
                                      (?, ?, ?, 'http://example.invalid/2', true, 'none', now())"
                               wh-1 tenant-a "Webhook A"
                               wh-2 tenant-a "Webhook B"])
      ...
      (let [rows (db.wh.outcomes/list-latest-outcomes-for-session ds tenant-a session-a {:limit 50})
        ...
        (is (= 2 (count rows)))
        (is (= 1 (:attempt_no row1)))
        (is (= 2 (:attempts_count row1)))
        (is (= "Webhook A" (:webhook_name row1)))
        (is (= "delivered" (:status row1)))
        (is (= 200 (:http_status row1)))
        (is (= 0 (:attempt_no row2)))
        (is (= 1 (:attempts_count row2)))
        (is (= "Webhook B" (:webhook_name row2)))))))
