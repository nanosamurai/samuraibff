...
as an ordered CSV selection. Omission selects `whisperx`; explicit IDs must
...
are generated by the existing Buf build. This spike exposes results through
the API; final-track settings/tabs and per-track completion UI are the later
selection/UI spike. The existing session status does not prove that every
...
Selection controls, track labels and browser tabs remain spike 3. Current
session status is not a per-track success/failure report. See
`docs/refinement-tracks-spike.md` for validation.
